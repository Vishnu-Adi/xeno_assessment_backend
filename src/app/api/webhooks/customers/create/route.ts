...
export async function POST(req: NextRequest) {
  ...
  const inserted = await prisma.$executeRawUnsafe('INSERT IGNORE INTO `WebhookEvent` (`tenantId`, `eventId`, `receivedAt`) VALUES (?, ?, NOW(6))', tenantId, eventId);
  if (inserted === 0) return NextResponse.json({ ok: true, deduped: true });
  await CustomersRepo.upsertFromShopify({ tenantId }, payload);
